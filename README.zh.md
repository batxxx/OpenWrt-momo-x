...
要确保安装到本仓库当前改造后的版本，可靠方式是从本仓库源码编译软件包，或者使用本仓库 GitHub Actions 构建出来的软件包产物。

在 `batxxx/OpenWrt-momo-x` 的软件源重新构建并发布之前，不要把公共软件源脚本当作主要安装方式。如果软件源仍然指向旧包，就会安装旧的上游版本，而不是当前改造版。
...
3. 下载生成的软件包 artifact。
4. 将 `.ipk` 或 `.apk` 软件包上传到路由器并手动安装。

### 从已发布软件源安装

只有在本仓库的软件源已经重新构建并发布后，才使用这种方式。默认脚本读取 `https://momomomo.pages.dev`；需要将新软件源发布到这里，或者运行脚本时用 `MOMO_REPOSITORY_URL` 指向你自己的软件源。
