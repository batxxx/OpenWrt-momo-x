...
要确保安装到本仓库当前改造后的版本，可靠方式是使用本仓库 GitHub Actions 构建出来的软件包产物，或者从本仓库源码自行编译软件包。

不要把公共软件源脚本当作主要安装方式。默认软件源 `https://momomomo.pages.dev` 是一个已经发布出来的 OpenWrt 软件包源，里面包含 `momo`、`luci-app-momo`、LuCI 语言包等软件包；它不是只包含 sing-box 内核。如果这个软件源仍然是旧上游代码构建出来的，就会安装旧页面和旧服务脚本。

#### 使用 GitHub Actions 产物安装

1. 打开本仓库的 `Actions`。
2. 运行 `build-packages` 工作流，选择对应的 OpenWrt 分支和架构。
3. 下载生成的软件包 artifact。
...
当前改造版暂时没有新的公共软件源。只有当你自己维护软件源，并且已经用本仓库重新构建和发布软件包后，才使用这一节。
...
### 维护者专用：从 Release 软件源安装

只有在本仓库的 release/install 软件源已经重新构建并发布后，才使用这种方式。
